...
def generate_files_for_year(year):
    ...
    while current_date <= end_date:
        files.append(f"AIS_{current_date.strftime('%Y_%m_%d')}.csv")
        current_date += delta

    return files

# Main processing function for a single file
def process_file(data_file, site, csv_raw_path, compressed_folder):
    try:
        ...
        csv_raw_path = os.path.join(csv_raw_path, data_file)
        # print(csv_raw_path)
        # return
        s_time = time.time()
        ...
        with open(csv_raw_path, "r") as f:
            reader = csv.reader(f)
            next(reader)  # Skip header
            for info in reader:
                ship_num = info[0]
                # if info[VESSEL_TYPE_FIELD] == '' or\
                #     int(info[VESSEL_TYPE_FIELD], base=10):
                #     continue
        ...
        s_time = time.time()
        compressed_path = os.path.join(compressed_folder, data_file)
...
compressed_folder = "compressed_csv_data/"

if not os.path.exists(compressed_folder):
    os.makedirs(compressed_folder)

files = generate_files_for_year(year)[0:31]
print(files)
# exit(0)
# print(files[0])
